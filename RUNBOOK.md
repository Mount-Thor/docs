...
- **API reference source:** `https://api.mountthor.com/openapi.json` —
  generated by admin-api (`utoipa`) and re-fetched by Mintlify on every
  docs build. No bot-PR sync needed.
...
The `/openapi.json` URL is re-fetched on every docs build. To force a
rebuild against a fresh spec, trigger any commit on `main` (a no-op edit
to a comment in `docs.json` works). Most users will pick up the change on
the next merged PR organically; explicit forces are rarely needed.
...
| Site config (theme, nav, OpenAPI URL) | `docs.json` |
