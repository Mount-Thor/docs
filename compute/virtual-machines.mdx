...
During alpha, create VMs from Mount Thor-provided images.
...
- Cold starts can take several minutes while the image is prepared on the host.
  Subsequent starts are faster when the image is already cached.
...
Save the following to a file called `vm.yaml`. Replace `{VM_NAME}` with the
name you want for your VM:
