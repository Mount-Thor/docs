...
VM `screen` port, and prints a VNC URI. Keep the command running while the
...
Use a VNC-compatible desktop client. The VM desktop session is passwordless
because Mount Thor authorizes access through your compute session and the
temporary `VirtualMachineDesktopSession`.
