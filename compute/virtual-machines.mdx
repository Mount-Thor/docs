...
```bash
mthr vm desktop --name dev --local-port 15900 --ttl 5m
```

The command opens a private tunnel to the VM screen port and automatically
launches Apple Screen Sharing. Keep the CLI running while your desktop client
is connected.

Use `--print` to print the VNC URL instead of opening it automatically:

```bash
mthr vm desktop --name dev --local-port 15900 --print
```
