...
- Mount Thor bills each bare-metal machine for a minimum of 24 hours,
  regardless of when you free it.
...
Open a local tunnel to a named port on the machine. Replace `{PORT_NAME}`
with the port name (e.g. `ssh`) and `{LOCAL_PORT}` with the port on your
workstation:
...
Save the following to a file called `machine.yaml`. Replace `{BM_NAME}` with
